...
class DDPMPipeline(DiffusionPipeline):
    ...
    def __call__(self,
                 ...
                 latent=False,
                 vae=None) -> Union[ImagePipelineOutput, Tuple]:
        r"""
        Args:
        ...
        image_shape = (batch_size, in_channels, sample_size, sample_size)
        self.device = "cpu" #ADDED
        ...
        if not latent:
            ...
            if vae is not None:
                image = (1 / vae.scaling_factor) * image
                if hasattr(vae, 'module'):
                    image = vae.module.decoder(image)
                else:
                    image = vae.decoder(image)
