...
from ai4ha.diffusion.models.unets.unet_2d_labels import UNet2DModel
...
def train(model, train_loader, image_key, BASE_DIR, num_epochs=10, alpha=10, device='cuda', opt=None, num_codes=256,
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            out, indices, cmt_loss = model(pixel_values)
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            with accelerator.accumulate(unet):
                pred_noise = unet(noised_latents, timesteps.to('cuda'), class_labels=(positions, label)).sample
...
def main(config):
    ...
    unet = UNet2DModel(
        class_embed_type = "timestep_label",
        **config["model"]["unet"]
    )
...
def LatentDDPM(cfg: DictConfig) -> None:
    ...
    cfg["name"] = cfg["model"]["modelname"] #'LatentDDPM-VQ256-1500'
    main(cfg)
