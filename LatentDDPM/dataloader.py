...
class PatientSliceDatasetGenerated(Dataset):
    def __init__(self, data_dir, transform=None):
        ...
        # Load all slices into a list
        for file_name in self.file_list:
            file_path = os.path.join(data_dir, file_name)
            data = np.load(file_path)
            volume = data['images'].astype(np.float32)  # Shape: (num_slices, num_channels, H, W)
            label = int(file_name[:1])    # Extract label
            for slice_idx in range(volume.shape[0]):  # Iterate over slices
                self.slices.append((volume[slice_idx], label))  # Store each slice with label

    def __len__(self):
        return len(self.slices)

    def __getitem__(self, idx):
        slice_img, label = self.slices[idx]
        slice_img = torch.tensor(slice_img)  # Convert to tensor
...
class NautiliusMRIDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        self.data_dir = data_dir
        self.transform = transform
        self.file_list = [f for f in os.listdir(data_dir) if f.endswith('.npz')]
    ...
    def __getitem__(self, idx):
        ...
        data = np.load(file_path)
        volume = data['data'].astype(np.float32)  # 3D MRI volume
        volume = (volume - volume.min()) / (volume.max() - volume.min())
        # Extract label from filename (first two digits)
        label = int(file_name[:2]) - 1

        volume = torch.tensor(volume[:181]).unsqueeze(0).permute(0, 1, 3, 2)  # Ensure single channel
